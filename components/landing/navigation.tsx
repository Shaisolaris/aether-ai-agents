  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Logo */}
          <a href="#" onClick={(e) => e.preventDefault()} className="flex items-center gap-2 group">
            <span className={`font-display tracking-tight transition-all duration-500 ${isScrolled ? "text-xl text-foreground" : "text-2xl text-white"}`}>Aether</span>
          ...
          <div className="hidden md:flex items-center gap-4">
            <a href="#" onClick={(e) => e.preventDefault()} className={`transition-all duration-500 ${isScrolled ? "text-xs text-foreground/70 hover:text-foreground" : "text-sm text-white/70 hover:text-white"}`}>
              Sign in
            </a>
            <Button
              size="sm"
              className={`rounded-full transition-all duration-500 ${isScrolled ? "bg-foreground hover:bg-foreground/90 text-background px-4 h-8 text-xs" : "bg-white hover:bg-white/90 text-black px-6"}`}
             onClick={() => toast.success("Agent deploying", { description: "Your AI agent is being provisioned." })}>
              Deploy agent
            </Button>
          </div>
